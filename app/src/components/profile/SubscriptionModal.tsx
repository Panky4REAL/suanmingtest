/* ============================================================
   玄机 · Pass 会员与特权订阅中心 (SubscriptionModal)
   - 灵童客 (Free) / 寻道者 (Pass) / 玄机天师 (Master) 三档特权
   - 权益对比表
   - 支持微信/支付宝付费开通与功德一键兑换
   ============================================================ */
...
export function SubscriptionModal({ onClose }: SubscriptionModalProps) {
  ...
  const handleRedeem = (tier: MembershipTier, cost: number, days: number, title: string) => {
    if (gongde < cost) {
      alert(`当前功德点数为 ${gongde}，不足 ${cost} 功德，请前往功德中心完成任务积攒，或直接使用微信/支付宝快速开通！`)
      return
    }
    addGongde(-cost, `兑换开通 · ${title}`)
    upgradeMembership(tier, days)
    alert(`🎉 恭喜！成功使用功德开通【${title}】！`)
  }
  ...
  return (
    <>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-[#789087] font-serif">
                  XuanJi Pass & Advanced Destiny Privileges
                </p>
              </div>
            </div>
            <button
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-serif-sc font-bold text-sm text-[#176f63]">
                    {membershipTier === 'master' ? '玄机天师 VIP (至尊永久)' : membershipTier === 'pro' ? '寻道者 Pass 会员' : '灵童客 (免费体验版)'}
                  </span>
                </div>
                <p className="text-[11px] text-[#8A5B21] font-serif mt-0.5">
                  {membershipTier === 'free' ? '可直接使用微信/支付宝快速开通，或使用任务功德免费兑换' : `权益状态：${formatExpiry(vipExpiresAt)}`}
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-5 rounded-3xl border-2 border-[#176f63] bg-gradient-to-b from-[#176f63]/5 to-[#fffdf9] flex flex-col justify-between space-y-4 shadow-sm relative">
              ...
              <div className="space-y-2">
                ...
                <div className="pt-2 flex items-baseline gap-1">
                  ...
                  <span className="text-2xl font-black font-mono text-[#176f63]">19.9</span>
                  <span className="text-xs text-[#55736b] ml-0.5 font-serif">/月 (或 100功德)</span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <button
                  onClick={() => setCheckoutPlanId('pass_yearly')}
                  className="btn-xuanji w-full py-2.5 rounded-xl text-xs font-bold font-serif-sc shadow-2xs flex items-center justify-center gap-1"
                >
                  <span>💳 立即开通 (微信/支付宝)</span>
                </button>
                <button
                  onClick={() => handleRedeem('pro', 100, 7, '寻道者 Pass 7天卡')}
                  className="w-full py-1.5 rounded-xl text-[11px] font-serif border border-[#176f63]/30 text-[#176f63] hover:bg-[#176f63]/10 transition-colors"
                >
                  使用 100 功德免费兑换 7 天
                </button>
              </div>
            ...
            <div className="p-5 rounded-3xl border border-[#c58a28] bg-gradient-to-b from-[#c58a28]/10 to-[#fffdf9] flex flex-col justify-between space-y-4 shadow-sm">
              <div className="space-y-2">
                ...
                <h4 className="text-xl font-bold font-serif-sc text-[#8A5B21]">玄机天师 VIP</h4>
                <p className="text-xs text-[#78592c] font-serif">
                  玄门大师级专属定制与流年万字详批
                </p>
                <div className="pt-2 flex items-baseline gap-1">
                  <span className="text-xs text-[#c58a28] font-bold">¥</span>
                  <span className="text-2xl font-black font-mono text-[#8A5B21]">298</span>
                  <span className="text-xs text-[#78592c] ml-0.5 font-serif">终身永久买断</span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <button
                  onClick={() => setCheckoutPlanId('master_lifetime')}
                  className="w-full py-2.5 rounded-xl bg-gradient-to-r from-[#c58a28] to-[#8A5B21] text-white text-xs font-bold font-serif-sc shadow-2xs hover:opacity-90 flex items-center justify-center gap-1"
                >
                  <span>🔱 终身永久开通 ¥298</span>
                </button>
                <button
                  onClick={() => handleRedeem('master', 300, 30, '玄机天师 30天卡')}
                  className="w-full py-1.5 rounded-xl text-[11px] font-serif border border-[#c58a28]/40 text-[#8A5B21] hover:bg-[#c58a28]/10 transition-colors"
                >
                  使用 300 功德兑换 30 天
                </button>
              </div>
      ...
      {/* 收银台弹窗 */}
      <CheckoutModal
        isOpen={Boolean(checkoutPlanId)}
        initialPlanId={checkoutPlanId || 'pass_yearly'}
